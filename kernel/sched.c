 ...
 *
 * 
 * A modifiable variable which we can set that determines
 * the number of ticks a process will receive in the scheduler
 * Change made by Jonas Schreiber
 *
 */
long OUR_TICKS = DEF_COUNTER;
 ...
 *
 * 
 * A spinlock that prevents more than one process from
 * receiving double time on the CPU
 * Change made by Jonas Schreiber
 *
 */
spinlock_t myincreaseshare_lock = SPIN_LOCK_UNLOCKED;
 ...
 */
	...
	if (unlikely(!c)) {
		...
		for_each_task(p){
			 ...
			 * 
			 */		
			p->user->user_divisor = 0;
			if(p->state == TASK_RUNNING){
				p->user->user_divisor += p->multiplier;
				if (max_user_divisor < p->user->user_divisor)
				  max_user_divisor = p->user->user_divisor;

			}
		}

		OUR_TICKS = max_user_divisor * DEF_COUNTER;
		for_each_task(p){
			/*
			 * The standard case
			 */
			 ...
			 *
			 * Change made by Jonas Schreiber
			 * The "special case" of multiplying time by two
			 */
			else if(p -> policy == SCHED_OTHER && p -> state == TASK_RUNNING)
				p -> counter = (p -> counter >> 1) + OUR_TICKS * (p -> multiplier / p -> user -> user_divisor);
 ...
 *
 * 
 * Code for sys_myincreaseshare
 * Change made by Jonas Schreiber
 *
 */

asmlinkage void sys_myincreaseshare(void){
  unsigned long flags;
  spin_lock_irqsave(&myincreaseshare_lock, flags);
  current -> multiplier *= 2;
  spin_unlock_irqrestore(&myincreaseshare_lock, flags);
  return;
}